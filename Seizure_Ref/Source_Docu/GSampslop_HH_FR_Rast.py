...
start_scope()
Cnt=0
for Nseed in [1]: #[i for i in range(5)]:
    for NAmp in [16]:#[0,18]: #,10,12,14,16,18]: #range(10): #2-12 ??
        for NbS in [4]: #[0,14]: #[0,2,4,6,8,10,12,14]: #range(15):
            NbSim=NbS
            Nsim=NbS
            Cnt=Cnt+1
            seed(Nseed)
     ...
     #       Lt2G2=array(M3G2.t/ms)
          ...
          #  plt.show()
            ...
            connexions = {"Poisson to inhib": np.column_stack((S_ed_in.i, S_ed_in.j)), \
                      ...
                      "S_12": np.column_stack((S_12.i, S_12.j)), \
                      "S_11": np.column_stack((S_11.i, S_11.j)), \
                      "S_21": np.column_stack((S_21.i, S_21.j)), \
