...
for Nseed in range(100):
    ...
    for NAmp in range(10):
        ...
        for NbS in range(10):
            ...
            # Population 2 - RS
            b2 = 100. * pA
            G2 = NeuronGroup(N2, eqs, threshold='v > -40.*mV', reset='v = -65*mV; w += b2', refractory='5*ms',
                             method='heun')
            G2.v = -65. * mV
            G2.w = 0.0 * pA
            G2.GsynI = 0.0 * nS
            ...
            def input_rate(t, t1_exc, tau1_exc, tau2_exc, ampl_exc, plateau):
                ...
                inp = ampl_exc * (np.exp(-(t - t1_exc) ** 2 / (2. * tau1_exc ** 2)) * heaviside(-(t - t1_exc)) +
                                  ...
                                  np.exp(-(t - (t1_exc + plateau)) ** 2 / (2. * tau2_exc ** 2)) * heaviside(
                            t - (t1_exc + plateau)))
                return inp
            ...
            np.save('Seizure_Ref/Results2/AD_popRateExc_Sim_' + str(TauP) + '_Amp_' + str(NAmp) + 'Nseed_' + str(
                Nseed) + '.npy', popRateG2)
