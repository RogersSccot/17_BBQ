...
for tau_S_AdEx in [2,3,4,5,6]:
    for tau_S_HH in [2,3,4,5,6,7,8,9]:
        for CP in [0.5]:
            for T_ref_HH in [10]:
                T_ref_AdEx=10
                # 输出当前时间
                ...
                HH_FS_neuron=HHNeuron(name="Ex_H1_1", V_Neuron=-65*mV, position=(0,0), G_Synapsis_Excitatory=0, G_Synapsis_Inhibitory=0, 
                                                ...
                                                Connecting_Neuron=[], Q_Synapsis=5*nS, Probability_Connecting=CP,
                ...
                neuron_potential=np.zeros((Num_Real, time_sim.shape[0]))
                neuron_potential_after=np.zeros((Num_Real, time_sim.shape[0]))
                neuron_name=[]
                ...
                output_path = os.path.join(output_folder, "All"+"_rest_"+str(rest)+"_AmpStim_"+str(AmpStim)+"_TauP_"+str(TauP)+"_tau_S_"+str(tau_S_AdEx)+"_"+str(tau_S_HH)+"_T_ref_"+str(T_ref_AdEx)+"_"+str(T_ref_HH)+"_Num_"+str(Num)+"_CP_"+str(CP)+"A.png")
                # output_path='My_Union_Plot/'+"All"+"_rest_"+str(rest)+"_AmpStim_"+str(AmpStim)+"_TauP_"+str(TauP)+"_tau_S_"+str(tau_S_AdEx)+"_"+str(tau_S_HH)+"_T_ref_"+str(T_ref_AdEx)+"_"+str(T_ref_HH)+"_Num_"+str(Num)+"_CP_"+str(CP)+"A.png"
                plt.savefig(output_path)

                fig2=plt.figure(figsize=(20,10))
                points_r = np.where(fire_matrix1[:,21000:25000] > 1)
                points_g = np.where(fire_matrix2[:,21000:25000] > 1)
                plt.scatter(points_r[1], N2+points_r[0], color='red',s=1)
                plt.scatter(points_g[1], points_g[0], color='green',s=1)

                output_folder = os.path.join(current_dir, 'My_Union_Plot')
                output_path = os.path.join(output_folder, "Micro"+"_rest_"+str(rest)+"_AmpStim_"+str(AmpStim)+"_TauP_"+str(TauP)+"_tau_S_"+str(tau_S_AdEx)+"_"+str(tau_S_HH)+"_T_ref_"+str(T_ref_AdEx)+"_"+str(T_ref_HH)+"_Num_"+str(Num)+"_CP_"+str(CP)+"A.png")
                # output_path='My_Union_Plot/'+"Micro"+"_rest_"+str(rest)+"_AmpStim_"+str(AmpStim)+"_TauP_"+str(TauP)+"_tau_S_"+str(tau_S_AdEx)+"_"+str(tau_S_HH)+"_T_ref_"+str(T_ref_AdEx)+"_"+str(T_ref_HH)+"_Num_"+str(Num)+"_CP_"+str(CP)+"A.png"
                plt.savefig(output_path)
                # close all the plot
                plt.close('all')

                output_folder = os.path.join(current_dir, 'My_Union_Data')
                output_path = os.path.join(output_folder, "All"+"_rest_"+str(rest)+"_AmpStim_"+str(AmpStim)+"_TauP_"+str(TauP)+"_tau_S_"+str(tau_S_AdEx)+"_"+str(tau_S_HH)+"_T_ref_"+str(T_ref_AdEx)+"_"+str(T_ref_HH)+"_Num_"+str(Num)+"_CP_"+str(CP)+"A.npz")
